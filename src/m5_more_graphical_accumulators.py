...
def draw_squares_from_circle(n, circle, window):
    ...
    for k in range(n):
        square1 = rg.Square(rg.Point(circle.center.x + circle.radius * k , circle.center.y + circle.radius * k), circle.radius * 2)
...
def run_test_draw_circles_from_rectangle():
    ...
    con1 = rg.Point(375, 330)
    con2 = rg.Point(350, 280)
    rec1 = rg.Rectangle(con1, con2)
...
def draw_circles_from_rectangle(m, n, rectangle, window):
    ...
    for k in range (m):
        circle1 = rg.Circle(rg.Point(rectangle.get_center().x - rectangle.get_width()/2 - rectangle.get_height()*(2*k+1)/2, rectangle.get_center().y), rectangle.get_height()/2)
        circle1.attach_to(window)
        circle1.fill_color = rectangle.fill_color
    window.render()

    for k in range (n):
        circle2 = rg.Circle(rg.Point(rectangle.get_center().x, rectangle.get_center().y - rectangle.get_height()/2 - rectangle.get_width()*(2*k+1)/2),rectangle.get_width()/2)
